...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			...
			'cat'				=> get_catId( et_get_option( 'deepfocus_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	if ( 'false' == et_get_option( 'deepfocus_blog_style', 'false' ) ) {
		...
		if ( $exclude_gallery_categories ) $query->set( 'category__not_in', $exclude_gallery_categories );
		
		$query->set( 'posts_per_page', et_get_option( 'deepfocus_portfolio_number', '8' ) );
		
		return;
	}
	
	/* Set the amount of posts per page on homepage */
	$query->set( 'posts_per_page', et_get_option( 'deepfocus_homepage_posts', '6' ) );
